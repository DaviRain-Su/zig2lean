...
export fn ziglean_crypto_p256_sign(
    seed: [*]const u8,
    message: [*]const u8,
    message_len: u64,
    out_signature: [*]u8,
    ...
    if (seed[0..32].len != 32) return STATUS_INVALID;
    const seed_bytes: *const [32]u8 = seed[0..32];
    const kp = EcdsaP256.KeyPair.generateDeterministic(seed_bytes.*) catch return STATUS_INVALID;
    const sig = kp.sign(message[0..@intCast(message_len)], null) catch return STATUS_INVALID;
    const encoded = sig.toBytes();
    @memcpy(out_signature[0..64], &encoded);
    return STATUS_OK;
...
export fn ziglean_crypto_p256_verify(
    public_key: [*]const u8,
    public_key_len: u64,
    message: [*]const u8,
    message_len: u64,
    ...
    const pk = EcdsaP256.PublicKey.fromSec1(public_key[0..@intCast(public_key_len)]) catch {
        out_valid.* = 0;
        return STATUS_OK;
    };
    const sig = EcdsaP256.Signature.fromBytes(signature[0..64].*);
    EcdsaP256.Signature.verify(sig, message[0..@intCast(message_len)], pk) catch {
        out_valid.* = 0;
        return STATUS_OK;
    };
    out_valid.* = 1;
