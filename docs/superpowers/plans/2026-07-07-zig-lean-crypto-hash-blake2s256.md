# Blake2s-256 Extension Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add Blake2s-256 to `ZigLean.Crypto.Hash`.
...
### Task 1: Add Blake2s-256 Zig implementation
...
git commit -m "feat(crypto): add Blake2s-256 in Zig"
```

---

### Task 2: Add Blake2s-256 C header declaration
...
git commit -m "feat(crypto): declare Blake2s-256 in header"
```

---

### Task 3: Add Blake2s-256 C shim
...
git commit -m "feat(crypto): add Blake2s-256 C shim"
```

---

### Task 4: Add Blake2s-256 Lean FFI and public API
...
git commit -m "feat(crypto): add Lean Blake2s-256 API"
```

---

### Task 5: Add Blake2s-256 tests
...
```lean
  let b2s_empty ← blake2s256 ByteArray.empty
  assertEq "blake2s256 empty length" b2s_empty.size 32
  expectHex "blake2s256 empty" b2s_empty
...
git commit -m "test(crypto): add Blake2s-256 tests"
```
...
**Spec coverage:**
- Blake2s-256 one-shot API → Tasks 1-4
- Tests → Task 5
...
**Type consistency:**
- `lean_ziglean_crypto_blake2s256` consistent across all layers.
